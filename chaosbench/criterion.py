...
class MAPE(nn.Module):
    def __init__(self, lat_adjusted=True, eps=1e-6):
    ...
    def forward(self, predictions, targets):
        ...
        percentage_error = torch.abs((predictions - targets) / (targets + self.eps))
        mean_absolute_percentage_error = torch.nanmean(percentage_error) * 100
...
class ACC(nn.Module):
    ...
    def __init__(self,
        ...
        self.normalization_mean = {
                'era5_single': xr.open_dataset(self.normalization_file['era5_single'], engine='zarr')['mean'],
                'era5_single': xr.open_dataset(self.normalization_file['era5_pre'], engine='zarr')['mean'],
    ...
    def forward(self, predictions, targets):
        ...
        # climatology = torch.tensor(self.normalization_mean[source].sel(param=param).values).to(targets.device)
        ...
        # Compute anomalies
        anomalies_targets = targets
        anomalies_predictions = predictions
