...
class VGAE(nn.Module):
    def __init__(self, input_dim, hidden_nf,output_dim,pred_len=2,edges_in_nf=0, act_fn=nn.SiLU(), n_layers=4, attention=0, recurrent=False):
        ...
        self.mean = GCL(self.hidden_nf, self.hidden_nf, self.hidden_nf, edges_in_nf=1,
                                              pred_len=pred_len,act_fn=act_fn, attention=attention, recurrent=recurrent)
        self.std = GCL(self.hidden_nf, self.hidden_nf, self.hidden_nf, edges_in_nf=1,
                                              pred_len=pred_len,act_fn=act_fn, attention=attention, recurrent=recurrent)
    ...
    def forward(self, nodes, edges, edge_attr=None):
        ...
        mean, _ = self.mean(h, edges, edge_attr=edge_attr)
        std, _ = self.std(h, edges, edge_attr=edge_attr)
        std = torch.clamp(std, max=10)
        kl_divergence = (0.5 / h.size(0)) * (mean ** 2 + torch.exp(std) ** 2 - 2 * std - 1).sum(1).mean()
        gaussian_noise = torch.randn(h.size(0), self.hidden_nf).to(h.device)
        h = mean + gaussian_noise * torch.exp(std)
        h = self.decoder(h)
        h = h.reshape(h.shape[0], self.pred_len, self.output_dim)
        return h, kl_divergence
